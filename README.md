...
**MediGo**는 AI 기반 약 배달 및 복약 지도 서비스로, 처방전부터 약 수령까지의 전체 과정을 디지털화합니다.

### 핵심 가치

🏠 **완전 무외출 서비스**
- 병원 진료 후 약국 방문 없이 집에서 약을 받을 수 있습니다
- 고령자, 감염성 질환 환자, 보호자에게 최적화된 서비스입니다

🤖 **AI 기반 맞춤형 복약 지도**
- OCR 기술로 처방전 정보를 자동 추출합니다
- AI가 개인 맞춤형 복약 지도를 생성하여 카카오톡으로 발송합니다
- 약봉투의 복잡한 정보를 이해하기 쉬운 언어로 변환합니다

📱 **카카오톡 기반 접근성**
- 앱 설치 없이 카카오톡 채널만으로 서비스 이용이 가능합니다
- 고령자도 쉽게 사용할 수 있는 친숙한 인터페이스입니다

### 시스템 아키텍처

**최종 목표**: 처방전 이미지 업로드 → OCR 자동 처리 → AI 복약 지도 생성 → 약 배달 → 카카오톡 메시지 발송의 **완전 자동화**

**MVP 단계 (현재)**: Wizard of Oz 방식으로 운영하며, 사용자에게 서비스 가치를 제공하면서 동시에 AI 모델 학습을 위한 데이터를 수집합니다.
- OCR은 자동 처리되지만, 복약 지도는 운영자가 수동 작성
- 수집된 데이터(OCR 텍스트 + 복약 지도 페어)로 향후 AI 모델 학습

**향후 계획**: 수집된 데이터를 바탕으로 LLaMA 3 기반 한국어 모델을 파인튜닝하여 복약 지도 생성 자동화
